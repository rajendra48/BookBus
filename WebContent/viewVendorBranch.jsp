							...
							<div class="box-content nopadding">
								<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper"
									...
									<table class="table table-nomargin dataTable table-bordered"
										...
										int totalCount=db.getBranchCount();
										
										int current=1;
										try
										{
											String s=request.getParameter("vendorBranch_pageCount");
										...
										ArrayList<Branch> branches=db.getBranch(current);
										%>
										...
										<thead>
											...
											<tr role="row">
												<th class="sorting_asc" role="columnheader" tabindex="0"
													...
													aria-label="Rendering engine: activate to sort column descending">Branch
													Name</th>

												<th class="sorting" role="columnheader" tabindex="0"
													aria-controls="DataTables_Table_0" rowspan="1" colspan="1"
													style="width: 237px;"
													aria-label="Browser: activate to sort column ascending">Vendor Name</th>
										...
										<tbody role="alert" aria-live="polite" aria-relevant="all">
											<%
										if(branches.size()>0)
										for(Branch v: branches)
										{
											%>
											<tr class="odd">
												<td class="  sorting_1"><%=v.getName() %></td>
												<td class=" "><%=db.getVendor(v.getVendori_id()+"").getName() %></td>
												<td class="hidden-350 "><%=v.getLocation() %></td>
												<td class="hidden-1024 "><%=v.getPhno() %></td>
												<td class="hidden-480 ">
													<table>
														<tr>
															<td>
																<form action="editBranch.jsp" method='post'>
																	<input type='hidden' name='branch_id'
																		value='<%=v.getBranch_id() %>' /> <input type='submit'
																		value='View and Edit' /> <input type='hidden'
																		name='vendorBranch_pageCount' value='<%=current %>' />
																</form>
															</td>
															<td>
																<form action='deleteBranch.jsp' method='post'>
																	<input type='hidden' name='vendorBranch_pageCount'
																		value='<%=current %>' /> <input type='hidden'
																		name='branch_id' value='<%=v.getBranch_id() %>' /> <input
									...
									<div class="dataTables_paginate paging_full_numbers"
										...
										else if((((current+1)*10)<((totalCount/10)*10))&&current==1)
										{
											...
											id="DataTables_Table_0_previous" style='background: #368ee0;'>Previous</a><span><a
												tabindex="0" class="paginate_active">1</a> <a tabindex="0"
												href="viewVendors.jsp?vendorBranch_pageCount=<%=current+1 %>"
										...
										else if((((current+1)*10)<((totalCount/10)*10))&&current>1)
										{
											...
											id="DataTables_Table_0_previous">Previous</a><span><a
												...
												href="viewVendors.jsp?vendorBranch_pageCount=<%=current-1 %>"><%=current-1 %></a>
